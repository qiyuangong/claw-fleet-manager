...
3. Edit `packages/server/server.config.json`:
   ...
   - Add a `profiles` block to enable profile instances (see example config for keys). Avoid using `main` as a profile name — OpenClaw reserves that name for the standalone default profile.
