...
- `docker`：管理现有 fleet 目录中的 `openclaw-N` 容器
...
- 启动、停止、重启和扩缩容容器
- 通过 `ComposeGenerator` 重建 `docker-compose.yml`
...
- 缩容时会优先停止编号更大的容器
- 状态收敛机制是 `docker compose up -d --remove-orphans`
...
- 在必要时从 `docker-compose.yml` 推导 `count`
- 将 token 脱敏后返回给前端
...
[`packages/server/src/services/compose-generator.ts`](../../packages/server/src/services/compose-generator.ts) 在 Docker 模式下重建 `docker-compose.yml`。

生成内容包括：

- 固定命名的 `openclaw-N` 服务
- 每实例独立的配置和 workspace 挂载
- 写入 `.env` 的网关 token
- 来自 `fleet.env` 的资源限制
- `read_only`、`tmpfs`、`cap_drop: ALL` 和 `no-new-privileges`
- 指向 `http://127.0.0.1:18789/healthz` 的健康检查
...
- `docker-compose.yml`：Docker 模式期望状态
