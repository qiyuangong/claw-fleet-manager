...
| Running Sessions | 实时监控全舰队当前活跃会话（仅管理员） |
| Manage Sessions | 包含筛选、排序和搜索的历史会话表格（仅管理员） |
...
4. 在弹出的对话框中输入新名称。
   ...
   > **命名规则：** Profile 实例使用小写字母、数字和连字符。Docker 实例遵循 `openclaw-N` 编号规则，不能自由重命名为任意字符串。
...
3. 使用状态和时间筛选器缩小列表范围。使用搜索框通过标题、key、模型或最近消息预览查找会话。

4. 使用工具栏中的视图切换在**看板视图**和**表格视图**之间切换。

> **提示：** 看板视图按状态列（运行中、已完成、失败、被终止/超时）分组显示会话。表格视图更适合按费用或 Token 数量排序。
...
1. 在侧边栏中点击 **Running Sessions**（位于 Admin 区域下）。

2. 点击 **Start Monitoring** 开始实时轮询。面板每 300 毫秒刷新一次。
...
6. 点击 **Stop Monitoring** 暂停实时轮询。

> **注意：** 监控状态（已启动/已停止）会通过浏览器 localStorage 在页面刷新后保持。
...
1. 在侧边栏中点击 **Manage Sessions**（位于 Admin 区域下）。
...
2. 仪表盘显示：
   - 包含运行中、已完成、失败、被终止/超时会话数量的状态汇总卡片
   - 包含筛选后会话列表的活动看板
   - 汇总卡片中可点击的状态分类——点击某个分类可将看板聚焦于该状态

3. 调整**状态筛选器**（全部 · 活跃 · 已完成 · 错误）和**时间窗口**（今天 · 最近 24 小时 · 最近 7 天 · 全部）来缩小视图范围。
